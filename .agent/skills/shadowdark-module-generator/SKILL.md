...
**Rename extracted images:** After extraction, images have temporary names. Use the LLM to analyze the PDF context and image content, then run:
...
- Start with an urgent problem that _demands_ action from the characters. Make the stakes immediate so players feel compelled to respond.
...
**Slime Crawler (1)** AC 13, HP 22, ATK 1 pseudopod +4 (1d8+2 acid), MV near (swim), S +2, D +1, C +3, I -4, W +0, Ch -2, AL N, LV 3 **Special Abilities:**
...
5. **Check for previous sessions**: If `campaigns/<campaign>/Session <number> - <Session Title>` folders exist, read them for the DM instructions for previous sessions.
...
- Expand module content into detailed, ready-to-run notes -- **Reference campaign files**: Use `campaigns/<campaign>/campaign_docs/CAMPAIGN.md` (if present) or `campaigns/River of Night/` files for setting-specific details, NPCs, and continuity -- **Reference Shadowdark rules**: Use files in `shadowdark-library/core--rules/` for accurate monster stats and game mechanics -- **Use monster reference**: Consult files in `shadowdark-library/monster/` for specific monster stat blocks
